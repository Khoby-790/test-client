...
import { createBlog, updateBlog } from "../../graphql/mutations/auth";
import {
  CreateBlog,
  CreateBlogVariables,
...
import {
  UpdateBlog,
  UpdateBlogVariables,
} from "../../graphql/mutations/__generated__/UpdateBlog";
import { GetBlogs_getBlogs } from "../../graphql/queries/__generated__/GetBlogs";

type Props = {};

const UpdatePost = (props: Props) => {
  const data = useLocation().state as GetBlogs_getBlogs;
  ...
  const [execute, { loading }] = useMutation<UpdateBlog, UpdateBlogVariables>(
    updateBlog
  );

  const onSubmit: SubmitHandler<UpdateBlogVariables> = (variables) => {
    execute({ variables })
      .then(({ data }) => {
